...
- `data/chroma`는 볼륨 마운트로 영속화되어 컨테이너를 재시작해도 색인
  데이터가 유지됩니다.
- **LAN 노출 주의**: `docker-compose.yml`의 포트 매핑(`7860`/`9090`/`3001`)은
  기본적으로 호스트의 모든 네트워크 인터페이스에 바인딩됩니다. 즉 카페/
  사무실처럼 다른 사람과 같은 Wi-Fi/LAN을 쓰는 환경에서 실행하면, 같은
  네트워크의 제3자가 `http://<내PC-IP>:3001`로 Grafana에 접속해 기본
  계정(admin/admin)으로 로그인할 수 있습니다. 신뢰할 수 없는 네트워크에서는
  실행을 피하거나, `docker-compose.yml`의 포트를 `"127.0.0.1:7860:7860"`
  형태로 바꿔 로컬호스트에만 바인딩하세요.
